    ...
    try:
       ...
       # print(f"Commit history for {repo_name} in the date range ({start_date} to {end_date}):")
        ...
        with open(output, 'w', encoding='utf-8') as file:
            file.write(f"Commit history for {repo_name} in the date range ({start_date} to {end_date}):\n\n")
            ...
            for commit in commits:        
                ...
                file.write(f"Commit Sha: {commit_details.commit.sha}, Author: {commit.author.name if commit.author is not None else 'Unknown'}, Date: {commit.commit.author.date.strftime('%Y-%m-%d')}, Commit Message: {commit_details.commit.message} \n")
...
for repo in repos:
    ...
    get_commit_history(repo.name, organization_name, access_token, start_date, end_date, output)
